...
class SaveManager:
    ...
    def save_game_enhanced(self, main_player: Player, all_players: Dict[str, Player],
        ...
        try:
            ...
            # Vérifie si le dossier existe déjà et demande confirmation
            save_path = self.base_save_directory / clean_save_name
            if save_path.exists():
                response = input(f"Une sauvegarde existe déjà pour {clean_save_name.replace('_', ' ')}. Voulez-vous l'écraser? (o/n): ")
                if response.lower() == 'o':
                    # Écrase : supprime complètement le dossier existant
                    shutil.rmtree(save_path)
                else:
                    # Crée un nouveau dossier avec numéro
    ...
    def _save_main_player(self, player: Player, save_id: str) -> None:
        ...
        if isinstance(player, dict):
            print(f"⚠️ Attention: joueur principal est un dictionnaire, reconversion en Player")
    ...
    def _save_active_players(self, active_players: Dict[str, Player], save_id: str) -> None:
        ...
        for name, player in active_players.items():
            # Vérification de type pour éviter les erreurs
            if isinstance(player, dict):
                print(f"⚠️ Attention: {name} est un dictionnaire, reconversion en Player")
                player = Player.from_dict(player)
    ...
    def load_game_enhanced(self, save_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Combine toutes les données avec vérification de type
            all_players = {}
            
            # Ajoute les joueurs actifs
            for name, player in active_players.items():
                if isinstance(player, dict):
                    print(f"⚠️ Conversion joueur actif {name} depuis dictionnaire")
                    all_players[name] = Player.from_dict(player)
                else:
                    all_players[name] = player
            
            # Ajoute les joueurs du monde
            for name, player in world_players.items():
                if isinstance(player, dict):
                    print(f"⚠️ Conversion joueur monde {name} depuis dictionnaire")
                    all_players[name] = Player.from_dict(player)
                else:
                    all_players[name] = player
    ...
    def _reconstruct_player_from_optimized(self, data: Dict) -> Player:
        ...
        if "stats" in data:
            player.stats.update_from_dict(data["stats"])
        if "elo" in data:
            player.elo = data["elo"]
        ...
        for week_str, tournaments in year_data.items():
            ...
            for tournament_data in tournaments:
                ...
                for player_name, round_reached in tournament_data.get("results", {}).items():
                    ...
                    # Vérification de type pour éviter les erreurs
                    if isinstance(ref_player, dict):
                        print(f"⚠️ Attention: joueur de référence {player_name} est un dictionnaire")
                        ref_player = Player.from_dict(ref_player)
                ...
                for match_data in tournament_data.get("match_results", []):
                    if isinstance(match_data, dict) and "winner_ref" in match_data:
                        ...
                        # Vérification de type pour winner_ref et loser_ref
                        if winner_ref and isinstance(winner_ref, dict):
                            winner_ref = Player.from_dict(winner_ref)
                        if loser_ref and isinstance(loser_ref, dict):
                            loser_ref = Player.from_dict(loser_ref)
